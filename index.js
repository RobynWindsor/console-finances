...
for (let i = 0; i < finances.length; i++) {
  ...
  if (finances[i + 1]) {
    ...
    if (monthlyChange > maxChange) {
      ...
      maxChangeMonth = finances[i + 1][0];
    }
    if (monthlyChange < minChange) {
      minChange = monthlyChange;
      minChangeMonth = finances[i + 1][0];
    }
  }
}
...
console.log(
  `Greatest Decrease in Profits/Losses: ${minChangeMonth} £ ${minChange}`
);

// CLACULATE GREATEST INCREASE IN CHANGES
...
//   if (finances[i + 1]) {
//     const next = finances[i + 1][1];
//     const monthDiff = next - currentMonth;
//   }
// } // calculate difference between current month and next month then store that differenc
